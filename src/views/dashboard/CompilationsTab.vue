  ...
  <div>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          ...
          <v-btn
            text
            color="primary"
            @click="showCompileDialog = !showCompileDialog"
          >
      ...
      <template v-slot:expanded-item="{ headers, item }">
        <td :colspan="headers.length" class="py-2">
          <div
            class="d-flex"
            v-if="item.status === 'failed' && item.kind === 'verification'"
          >
            <v-spacer></v-spacer>
            <v-btn
              text
          ...
          <v-list v-if="item.status !== 'pending'" dense>
            <template v-for="(result, i) in item.results">
              ...
              >
                <v-list-item-avatar>
                  <v-icon small :color="getStatusColor(result.status)">{{
                    getStatusIcon(result.status)
                  }}</v-icon>
                </v-list-item-avatar>
                <v-list-item-content>
                  <v-list-item-title>{{ result.path }}</v-list-item-title>
                  <v-list-item-subtitle v-if="result.language">{{
                    result.language
                  }}</v-list-item-subtitle>
                  <v-list-item-subtitle v-if="result.error">{{
                    result.error
                  }}</v-list-item-subtitle>
                </v-list-item-content>
                <v-list-item-action
                  v-if="item.kind === 'deployment' && item.status === 'success'"
                >
                  <v-tooltip left>
    ...
    >
      ...
      <template v-slot:item.sources="{ item }">
        <span class="overline">{{
          helpers.plural(item.sources.length, "source file")
        }}</span>
      </template>
    </v-data-table>
