<template>
  <v-app-bar app clipped-left elevation="0" class="home-app-bar">
    ...
    <v-menu open-on-hover bottom offset-y v-if="!isAuthorized && isPrivate">
      ...
      <v-list>
        <v-list-item class="d-flex align-center" :href="`${config.API_URI}oauth/github/login`">
          <v-list-item-title>
            <v-icon class="mr-4">mdi-github</v-icon>GitHub
          </v-list-item-title>
        </v-list-item>
        <v-list-item class="d-flex align-center" :href="`${config.API_URI}oauth/gitlab/login`">
          <v-list-item-title>
