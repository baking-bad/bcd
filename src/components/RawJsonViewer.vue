...
export default {
  ...
  watch: {
    show(newValue) {
      ...
      let promise = null;
      const level = this.level || 'head';

      if (this.type === 'operation') {
        promise = this.rpc.getOperation(this.network, level, this.hash);
      } else if (this.type === 'big_map_value') {
        promise = this.rpc.getBigMapValue(this.network, level, this.ptr, this.keyhash);
      } else if (this.type === 'code') {
        promise = this.rpc.getContractCode(this.network, level, this.address);
      } else if (this.type === 'storage') {
        promise = this.rpc.getContractStorage(this.network, level, this.address);
      } else {
